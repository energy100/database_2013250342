...
<header>
	...
	<nav>
		<ol>
			...
			<li><a href = "http://localhost:8080/php_study/Board.php">LogOut</a></li>
		</ol>
	</nav>
	...
	<article>
		...
		if(empty($_POST['user']) === false){
		  ...
		  //user한테 database 쿼리
		  $result = mysql_query("SELECT studentName, studentID, password, level FROM student WHERE studentID = '$studentID' and password = '$password'");

		  $row = mysql_fetch_array($result);
		  if ($row['studentID'] == $studentID && $row['password'] == $password){
		    echo '<h3>'.$row['studentName'].'</h3>';
				echo "Yout ID : ".'<p>'.$row['studentID'].'</p>';
				echo "Your password: ".'<p>'.$row['password'].'</p>';
				echo "Your Level : ".'<p>'.$row['level'].'</p>';
